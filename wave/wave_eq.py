...
class PINN_wave(DeepModelSingle):
    ...
    def train(self, X_u_train, X_f_train, u_train, Loss_data, Loss_PDE, weight, Optimizer, log_loss):
...
def run_experiment(epoch_num, noise_type, noise, loss_type, N, weight, _data=[], abnormal_size=0):
    ...
    for it in range(adam_iter):
        ...
        Net_model.train(X_u_train, X_f_train, u_train, Loss_data, Loss_PDE, weight, Optimizer, log_loss)
    ...
    error_u, error_c = Net_model.predict_error()
    Net_model.plot_result(X_u_train, path.joinpath(f'{epoch_num}_{loss_type}_{N}_{noise_type}_{noise}_{abnormal_size}_{weight}.png'))
