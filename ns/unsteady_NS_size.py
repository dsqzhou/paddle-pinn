"""
while true; do python unsteady_NS_size.py --adam_iter=10000 --save_path='./data/test_size';done
"""
...
if __name__ == '__main__':
    ...
    for r in range(repeat):
        for noise_type in ['outlinear']:
            for noise in [0.0]:
                for abnormal_ratio in [0.05]:
                    for N in [8000, 6000, 4000, 2000]:
                        _data = []
    ...
    for r in range(repeat):
        for noise_type in ['outlinear']:
            for noise, abnormal_ratio in zip([0.05], [0.05]):
                for N in [8000, 6000, 4000, 2000]:
                    _data = []
                    abnormal_size = int(N * abnormal_ratio)
                    if executed_flag:
